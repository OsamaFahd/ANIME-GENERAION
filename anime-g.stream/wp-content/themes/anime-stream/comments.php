<div class="comenter"><h3><?php comments_number( '0 Comments' ); ?><div class="crt" style="float: right; font-size: 19px; margin-top: -2px;"><i class="fa fa-comments" aria-hidden="true"></i></div></h3></div>
<div class='disqusmen'>
<div class="clear"></div>
<div style="padding: 7px 10px; border-radius: 3px; font-weight: bold; display: block; background: #34495e; color: #fff; text-shadow: none; margin-top: 10px; line-height: normal;">املأ البريد الالكرتوني والاسم، او قم بالتسجيل</div>
<div class="comment-form">
            <h3 id='comments'> اترك تعليق </h3>
...
<?php if ( get_option('comment_registration') && !$user_ID ) : ?>	<p>You must be <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php echo urlencode(get_permalink()); ?>">logged in</a> to post a comment.</p>
	...
	<?php endif; ?>
		...
		<p><textarea name="comment" id="comment" placeholder="اكتب تعليقك..." cols="100%" rows="10" tabindex="4"></textarea></p>
		<input type="text" name="author" id="author" placeholder="الاسم..." value="<?php echo $comment_author; ?>" size="22" tabindex="1" <?php if ($req) echo "aria-required='true'"; ?> 
		<p><input type="text" name="email" id="email" placeholder="البريد الالكتروني..." value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> />
<div class="kaskusonclick" style="margin-top: 5px;"><span id="kaskusemoticonslink" style="cursor: pointer; text-align: center; margin: 5px 0; padding: 7px 10px; border-radius: 3px; font-weight: bold; display: block; background: #293c4e; color: #fff; text-shadow: none;" onclick="kaskusemoticonsclink()">[+] اضف رموز تعبيرية</span><button name="submit" type="submit" id="submit">ارسال</button></div>
