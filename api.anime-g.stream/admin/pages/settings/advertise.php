            ...
            $html->element("div", array("class" => "row"), array(
                $html->element("div", array("class" => "col-lg-4 col-md-6"), array(
                    $html->element("div", array("class" => "form-group"), array(
                        $html->label("skip_ad_opt", "control-label", "Skip Ads"),
                        $html->tip("Whether to show the popup ad code in streaimg & download page"),
                        $html->element("select", array("class" => "form-control", "id" => "skip_ad_opt", "name" => "skip_ad"), array(
                            $html->option("Enable", "enable", Data::Get("skip_ad")),
                            $html->option("Disable", "disable", Data::Get("skip_ad")),
                        )),
                    )),
                    $html->element("div", array("class" => "form-group"), array(
                        $html->label("skip_ad_code", "control-label", "Skip AD Code"),
                        $html->tip("Skip ad code to show  in streaimg"),
                        $html->element("textarea", array("class" => "form-control", "id" => "skip_ad_code", "name" => "skip_ad_code", "rows" => "6"), array(
                            base64_decode(Data::Get("skip_ad_code")),
                        )),
                    )),
                )),
...
$html->script('
    $("#skip_ad_opt").bind("change",function(){
        $value = $(this).val();
        if ($value == "enable") {
            $("#skip_ad_code").prop("disabled", false);
        } else {
            $("#skip_ad_code").prop("disabled", true);
        }
    });
